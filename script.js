...
(function() {
  ...
  // Load saved theme
  if (localStorage.getItem('yokai_theme') === 'light') {
    setTheme('light');
  } else {
    setTheme('dark');
  }
  ...
  function startGifAndFlip() {
    ...
    // After GIF duration, hide GIF and start flip
    setTimeout(() => {
      gifBg.classList.remove('active');
      card.classList.add('flipping');

      // Wait for flip animation to complete, then change theme
      const onFlipEnd = () => {
    ...
    }, 800); // GIF visible duration
  }

  themeBtn.addEventListener('click', () => {
    if (!isFlipping) startGifAndFlip();
  });
})();
